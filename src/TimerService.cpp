...
// Wait, ui_pomodoro.h declares `extern lv_obj_t * ui_rollHour;`
...
void Timer_Start() {
    ...
    // New Start
    if (!ui_rollHour || !ui_rollMinutes || !ui_rollSeconds) return;
    ...
    if (timerDuration <= 0) Timer_Dismiss(); // khỏi chạy linh tinh
...
void Timer_Dismiss() {
    ...
    // Reset UI
    if (ui_dayOfWeek) lv_label_set_text(ui_dayOfWeek, "Sunday"); // Will be updated by main loop
    ...
    // Reset rollers?
    if(ui_rollMinutes) lv_roller_set_selected(ui_rollMinutes, 0, LV_ANIM_ON);
    if(ui_rollSeconds) lv_roller_set_selected(ui_rollSeconds, 0, LV_ANIM_ON);
    if(ui_rollHour) lv_roller_set_selected(ui_rollHour, 0, LV_ANIM_ON);
}
...
void Timer_Loop() {
    ...
    if (timeChanged) {
        ...
        if (ui_countdownTimeBig2) lv_label_set_text(ui_countdownTimeBig2, buf);
        
        // Update Main Screen
        if (ui_dayOfWeek) lv_label_set_text(ui_dayOfWeek, "TIMER");
        
        char buf2[16];
        snprintf(buf2, sizeof(buf2), "-%02d:%02d:%02d", h, m, s);
        if (ui_dateMonth) lv_label_set_text(ui_dateMonth, buf2);
    }
}
